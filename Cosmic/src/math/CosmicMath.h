...
#define FLOATING_POINT_ERROR_PRESCION 0.0001f
	...
	template <typename T>
	inline String ToString(const Vec2<T> &a)
	{
		StringStream ss;
	...
	template <typename T>
	inline String ToString(const Vec3<T> &a)
	{
		StringStream ss;
	...
	template <typename T>
	inline Vec3<T> StringToVec3(String s)
	{
		s = s.substr(1, s.length() - 2);

		int32 first = static_cast<int32>(s.find_first_of(";"));
		int32 last = static_cast<int32>(s.find_last_of(";"));

		String x = s.substr(0, first);
		String y = s.substr(first + 1, last - first - 1);
		String z = s.substr(last + 1);

		Vec3<T> result = Vec3<T>(std::stof(x), std::stof(y), std::stof(z));
...
#pragma warning(pop)
	...
	template <typename T>
	Quat<T> Rotate(const T &d_angle, Vec3<T> axis)
	{
	...
	template <typename T>
	inline String ToString(const Quat<T> &a)
	{
		StringStream ss;
		ss << '(' << '{' << a.x << ";" << a.y << ";" << a.z << '}' << ' ' << a.w << ')';
		return ss.str();
	...
	template <typename T>
	inline Quat<T> StringToQuat(String s)
	{
		s = s.substr(2, s.length() - 3);

		int32 first = static_cast<int32>(s.find_first_of(";"));
		int32 mid = static_cast<int32>(s.find_last_of(";"));
		int32 last = static_cast<int32>(s.find_last_of("}"));

		String x = s.substr(0, first);
		String y = s.substr(first + 1, mid - first - 1);
		String z = s.substr(mid + 1, last - mid - 1);
		String w = s.substr(last + 1);

		Quat<T> result = Quat<T>(std::stof(x), std::stof(y), std::stof(z), std::stof(w));
	...
	inline String ToString(const Mat2<T> &a)
	{
		...
		String space = "            ";
		ss << "| " << Round(a.ptr[0]) << space << Round(a.ptr[1]) << " |" << '\n';
		ss << "| " << Round(a.ptr[2]) << space << Round(a.ptr[3]) << " |" << '\n';
